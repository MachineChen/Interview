...
事务遵循ACID原则，A是原子性（atomicity），这些操作应该要么完整地执行，要么完全不执行；B是一致性（consistency），事务执行前后涉及到的相关数据保持一致；C是隔离性（isolation），事务正确提交之前结果不应被其他事务所见；D是持久性（durability），执行结果得到持久保存。
...
PlatformTransactionManager 用于执行具体的事务操作。接口定义：
...
PlatformTransactionManager.getTransaction(…) 方法返回一个 TransactionStatus 对象。返回的TransactionStatus 对象可能代表一个新的或已经存在的事务（如果在当前调用堆栈有一个符合条件的事务）。TransactionStatus 接口提供了一个简单的控制事务执行和查询事务状态的方法。该接口定义：
...
根据PlatformTransactionManager、TransactionDefinition 和 TransactionStatus 三个核心接口，我们完全可以通过编程的方式来进行事务管理。示例代码：
...
基于 TransactionTemplate 的事务管理示例代码
...
基于 TransactionTemplate 的事务管理示例配置文件
